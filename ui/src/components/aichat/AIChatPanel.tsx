...
import { CircleStop, Loader2, Plus, SendHorizontal, Trash2, X } from "lucide-react";
import {
    AlertDialog,
    AlertDialogAction,
    AlertDialogCancel,
...
export const AIChatPanel = () => {
    ...
    return (
        <div className="bg-card text-foreground border-border fixed inset-0 z-50 flex flex-col overflow-hidden border shadow-xl sm:static sm:h-[32rem] sm:w-96 sm:rounded-lg">
                    ...
                    {sessions.map((s) => (
                        <div key={s.id}
                             className={`flex shrink-0 items-center gap-1 rounded px-2 py-1 text-xs ${
                                 s.id === activeSessionId ? "bg-accent text-accent-foreground" : "bg-muted"}`}>
                            ...
                            <AlertDialog>
                                <AlertDialogTrigger
                                    render={<button type="button" title={t("aiChat.deleteChat")} />}>
                                    <Trash2 className="h-3 w-3" />
                                </AlertDialogTrigger>
                                <AlertDialogContent>
                                    <AlertDialogHeader>
                                        <AlertDialogTitle>{t("aiChat.deleteChat")}</AlertDialogTitle>
                                        <AlertDialogDescription>
            ...
            <div className="flex-1 space-y-2 overflow-y-auto p-3">
                {messages.length === 0 && (
                    <p className="text-muted-foreground text-sm">{t("aiChat.empty")}</p>
                )}
            ...
            <form onSubmit={sendMessage} className="border-border flex gap-2 border-t p-3">
                <input
                    className="bg-muted focus-visible:ring-ring flex-1 rounded px-3 py-2 text-sm outline-none focus-visible:ring-2"
                    value={input}
                ...
                />
                {streaming ? (
                    // The AbortController was already wired; only the button was missing.
                    <button type="button" title={t("aiChat.stop")} onClick={() => abortRef.current?.abort()}>
                        <CircleStop className="h-5 w-5" />
                    </button>
                ) : (
                    <button type="submit" title={t("aiChat.send")} disabled={!input.trim()}>
