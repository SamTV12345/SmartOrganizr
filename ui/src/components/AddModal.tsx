...
import {mergeNewAuthorInList} from "../utils/AuthorUtilList";
...
export const AddModal:FC<ModalProps>  = ({headerText,children, onAccept,acceptText})=>{
    const openModal  = useAppSelector(state=>state.modalReducer.openAddModal)
    ...
    return  openModal ? <div id="defaultModal" tabIndex={-1} aria-hidden="true" className="overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 w-full md:inset-0 h-modal md:h-full z-40">
    <div className="grid place-items-center h-screen">
    <div className="relative rounded-lg shadow bg-gray-700 justify-center">
    <div className="flex justify-between items-start p-4 rounded-t border-b border-gray-600">
    <h3 className="text-xl font-semibold text-white">
        {headerText}
        </h3>
        <button type="button" className="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center hover:bg-gray-600 hover:text-white" data-modal-toggle="defaultModal" onClick={()=>dispatch(setOpenAddModal(false))}>
    ...
    <button data-modal-toggle="defaultModal" type="button" className="text-gray-500 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 bg-gray-700 text-gray-300 border-gray-500 hover:text-white hover:bg-gray-600 focus:ring-gray-600" onClick={()=>dispatch(setOpenAddModal(false))}>{t('cancel')}</button>
        <button data-modal-toggle="defaultModal" type="button" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center bg-blue-600 hover:bg-blue-700 focus:ring-blue-800" onClick={()=>createAuthor(createdAuthor)}>{acceptText}</button>
    </div>
    </div>
    </div>
    </div>:<div></div>
