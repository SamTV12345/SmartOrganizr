...
const createClubAbsence = `-- name: CreateClubAbsence :exec

INSERT INTO club_absence (id, club_id, user_id, start_date, end_date, reason)
VALUES (?, ?, ?, ?, ?, ?)
`

type CreateClubAbsenceParams struct {
	ID        string
	ClubID    string
	UserID    string
	StartDate time.Time
...
// ==== member absence ====
func (q *Queries) CreateClubAbsence(ctx context.Context, arg CreateClubAbsenceParams) error {
	_, err := q.db.ExecContext(ctx, createClubAbsence,
		arg.ID,
		arg.ClubID,
		arg.UserID,
		arg.StartDate,
...
const createEventProgramEntry = `-- name: CreateEventProgramEntry :exec
INSERT INTO club_event_program (id, event_id, note_id, title, position, duration_minutes, note_text)
VALUES (?, ?, ?, ?, ?, ?, ?)
`

type CreateEventProgramEntryParams struct {
	ID              string
	EventID         string
...
func (q *Queries) CreateEventProgramEntry(ctx context.Context, arg CreateEventProgramEntryParams) error {
	_, err := q.db.ExecContext(ctx, createEventProgramEntry,
		arg.ID,
		arg.EventID,
		arg.NoteID,
		arg.Title,
...
const deleteClubAbsence = `-- name: DeleteClubAbsence :execrows
DELETE FROM club_absence WHERE id = ? AND club_id = ? AND user_id = ?
`

type DeleteClubAbsenceParams struct {
	ID     string
	ClubID string
	UserID string
}

func (q *Queries) DeleteClubAbsence(ctx context.Context, arg DeleteClubAbsenceParams) (int64, error) {
	result, err := q.db.ExecContext(ctx, deleteClubAbsence, arg.ID, arg.ClubID, arg.UserID)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected()
...
const deleteEventProgram = `-- name: DeleteEventProgram :exec
DELETE FROM club_event_program WHERE event_id = ?
`

func (q *Queries) DeleteEventProgram(ctx context.Context, eventID string) error {
	_, err := q.db.ExecContext(ctx, deleteEventProgram, eventID)
	return err
}
...
const deleteIncompleteInventorySweep = `-- name: DeleteIncompleteInventorySweep :execrows
DELETE FROM inventory_sweep WHERE id = ? AND user_fk = ? AND completed_at IS NULL
`

type DeleteIncompleteInventorySweepParams struct {
	ID     string
	UserFk string
}
...
func (q *Queries) DeleteIncompleteInventorySweep(ctx context.Context, arg DeleteIncompleteInventorySweepParams) (int64, error) {
	result, err := q.db.ExecContext(ctx, deleteIncompleteInventorySweep, arg.ID, arg.UserFk)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected()
...
const findAuthorsByUserAndIds = `-- name: FindAuthorsByUserAndIds :many
SELECT id, extra_information, name, user_id_fk, wikidata_id, birth_year, death_year FROM authors WHERE user_id_fk = ? AND id IN (/*SLICE:ids*/?)
`

type FindAuthorsByUserAndIdsParams struct {
	UserIDFk sql.NullString
	Ids      []string
}

func (q *Queries) FindAuthorsByUserAndIds(ctx context.Context, arg FindAuthorsByUserAndIdsParams) ([]Author, error) {
	query := findAuthorsByUserAndIds
...
const getClubAbsenceByID = `-- name: GetClubAbsenceByID :one
SELECT id, club_id, user_id, start_date, end_date, reason, created_at FROM club_absence WHERE id = ? AND club_id = ?
`

type GetClubAbsenceByIDParams struct {
	ID     string
	ClubID string
}

func (q *Queries) GetClubAbsenceByID(ctx context.Context, arg GetClubAbsenceByIDParams) (ClubAbsence, error) {
	row := q.db.QueryRowContext(ctx, getClubAbsenceByID, arg.ID, arg.ClubID)
	var i ClubAbsence
	err := row.Scan(
		&i.ID,
		&i.ClubID,
...
SELECT a.id, a.club_id, a.user_id, a.start_date, a.end_date, a.reason, a.created_at, u.firstname, u.lastname, u.username
FROM club_absence a
...
type ListClubAbsencesRow struct {
	ID        string
	ClubID    string
	UserID    string
	StartDate time.Time
...
func (q *Queries) ListClubAbsences(ctx context.Context, clubID string) ([]ListClubAbsencesRow, error) {
	rows, err := q.db.QueryContext(ctx, listClubAbsences, clubID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListClubAbsencesRow
	for rows.Next() {
		var i ListClubAbsencesRow
		if err := rows.Scan(
			&i.ID,
			&i.ClubID,
			&i.UserID,
...
const listClubAbsencesCoveringDate = `-- name: ListClubAbsencesCoveringDate :many
SELECT user_id FROM club_absence
WHERE club_id = ?
  AND start_date <= ? AND end_date >= ?
`

type ListClubAbsencesCoveringDateParams struct {
	ClubID string
	OnDate time.Time
}

func (q *Queries) ListClubAbsencesCoveringDate(ctx context.Context, arg ListClubAbsencesCoveringDateParams) ([]string, error) {
	rows, err := q.db.QueryContext(ctx, listClubAbsencesCoveringDate, arg.ClubID, arg.OnDate, arg.OnDate)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const listEventProgram = `-- name: ListEventProgram :many
...
func (q *Queries) ListEventProgram(ctx context.Context, eventID string) ([]ClubEventProgram, error) {
	rows, err := q.db.QueryContext(ctx, listEventProgram, eventID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ClubEventProgram
	for rows.Next() {
		var i ClubEventProgram
		if err := rows.Scan(
			&i.ID,
			&i.EventID,
			&i.NoteID,
...
const listMyClubAbsences = `-- name: ListMyClubAbsences :many
SELECT id, club_id, user_id, start_date, end_date, reason, created_at FROM club_absence
WHERE club_id = ? AND user_id = ?
ORDER BY start_date
`

type ListMyClubAbsencesParams struct {
	ClubID string
	UserID string
}

func (q *Queries) ListMyClubAbsences(ctx context.Context, arg ListMyClubAbsencesParams) ([]ClubAbsence, error) {
	rows, err := q.db.QueryContext(ctx, listMyClubAbsences, arg.ClubID, arg.UserID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ClubAbsence
	for rows.Next() {
		var i ClubAbsence
		if err := rows.Scan(
			&i.ID,
...
const updateClubMemberAuthorized = `-- name: UpdateClubMemberAuthorized :exec
UPDATE club_participant
SET authorized = ?
WHERE club_id = ? and user_id = ?
`

type UpdateClubMemberAuthorizedParams struct {
	Authorized bool
	ClubID     string
	UserID     string
}

func (q *Queries) UpdateClubMemberAuthorized(ctx context.Context, arg UpdateClubMemberAuthorizedParams) error {
	_, err := q.db.ExecContext(ctx, updateClubMemberAuthorized, arg.Authorized, arg.ClubID, arg.UserID)
	return err
}
